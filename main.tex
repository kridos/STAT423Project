...
\noindent Since the distribution of the response variable is approximately normal, we deemed a transformation as unnecessary. 
...
\begin{enumerate}
  ...
  \item Determine whether the interaction effects model includes substatial predictors that weren't
  included in our main effects model by running an anova test at a signficance
  level of 0.1
  \item If the p-value is significant, run a backwards step procedure (based on AIC) on the interaction effects model. Otherwise, run the backwards step
